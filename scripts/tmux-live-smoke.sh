...
Coverage:
  ...
  - basic non-interactive prompt, with one bounded retry for empty timeout exits
  - default ambient settings prompt
  - simple non-interactive math prompt
  - interactive TUI output/footer polling with cleanup
