...
- When pi-cursor-sdk registers native replay wrappers for `read`, `grep`, `find`, `bash`, `edit`, `write`, and `ls`, those calls render as compact one-line rows in OpenCode style (`→ Read …`, `✱ Grep "…" in …`, `$ command`, `← Edit/Write …`, `→ List …`) with `renderShell: "self"` instead of boxed tool cards. Completed grep/find rows append `(N matches)`; other output stays hidden until expanded except collapsed `edit`/`write` previews (OpenCode-style syntax-highlighted unified diffs with line-number gutters, per-line added/removed/context backgrounds, bounded under the call line); errors and image-read hints still render collapsed. See `src/cursor-compact-tool-display.ts`, `src/cursor-compact-diff-display.ts`, and `docs/cursor-native-tool-replay.md`. Set `PI_CURSOR_NATIVE_TOOL_DISPLAY=0` to fall back to scrubbed Cursor activity transcripts instead.
