...
- When `cursorCompactToolDisplay` is enabled in settings (or `PI_CURSOR_COMPACT_TOOL_DISPLAY=1` for one-off sessions) on a Cursor SDK session and pi-cursor-sdk registers native replay wrappers for `read`, `grep`, `find`, `bash`, `edit`, `write`, and `ls`, those calls render as compact one-line rows in OpenCode style (`→ Read …`, `✱ Grep "…" in …`, `$ command`, `← Edit/Write …`, `→ List …`) with `renderShell: "self"` instead of boxed tool cards. The setting does not change tool UI for non-Cursor models/providers. Without it, native replay still uses pi's default boxed tool shells. Completed grep/find rows append `(N matches)`; other output stays hidden until expanded except collapsed `edit`/`write` previews (OpenCode-style syntax-highlighted unified diffs with line-number gutters, per-line added/removed/context backgrounds, bounded under the call line) and compact error blocks (red-highlighted call plus up to five output lines, expandable for full output). `read` image results render inline when expanded; collapsed image reads keep the expand hint. Cursor `task`/subagent replay keeps task output previews in the tool row (widget UI remains separate). See `src/cursor-compact-tool-display.ts`, `src/cursor-compact-diff-display.ts`, `src/cursor-compact-output-display.ts`, and `docs/cursor-native-tool-replay.md`. Set `PI_CURSOR_NATIVE_TOOL_DISPLAY=0` to fall back to scrubbed Cursor activity transcripts instead.
