...
- Bridge diagnostics are opt-in only: `PI_CURSOR_PI_TOOL_BRIDGE_DEBUG=1` writes scrubbed single-line JSONL records to `process.stderr` with prefix `[pi-cursor-sdk:bridge]`. Diagnostics include run lifecycle, exposed pi↔MCP name pairs, queued requests, result resolution, rejection, cancellation, and pending counts. They must not include endpoint paths/URLs/tokens, API keys, bearer tokens, cookies, raw args/results, stdout/stderr payloads, file contents, or Cursor settings output, and they must not call pi UI status, notification, or footer APIs.
...
4. Tool bridge and replay:
   ...
   - confirm `PI_CURSOR_PI_TOOL_BRIDGE_DEBUG=1` emits scrubbed JSONL to `process.stderr` with prefix `[pi-cursor-sdk:bridge]`, and unset/false leaves output unchanged
