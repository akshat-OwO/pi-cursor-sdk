...
- When pi-cursor-sdk owns native replay wrappers for `read`, `grep`, and `find`, those calls render as compact one-line rows (`→ Read …`, `→ Grep … in …`, `→ Find … in …`) with `renderShell: "self"` instead of boxed tool cards. Output stays hidden until expanded; errors and image-read hints still render collapsed. See `src/cursor-compact-tool-display.ts` and `docs/cursor-native-tool-replay.md` for the global `compact-tool-display` extension conflict rules.
