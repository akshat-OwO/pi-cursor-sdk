...
- Cursor-side thinking remains visible. Cursor internal tool activity is recorded from SDK events, scrubbed, and replayed through pi's native `ToolExecutionComponent` inside a custom display message where possible (for example green `read <path>` or `$ <command>` cards). These displays do not force Cursor to use pi tools and do not re-execute Cursor's reads or shell commands. Cursor text deltas are treated as intermediate progress unless the final run result is unavailable, so the saved visible answer stays clean while Cursor's own tool calls remain Cursor-owned.
