...
import { getCursorPiBridgeContractText } from "../bridge/cursor-bridge-contract.js";
import { getCursorReplayPromptLabel } from "../transcript/cursor-tool-names.js";
...
function applyPromptBudget(
	...
	if (maxInputTokens === undefined || !Number.isFinite(maxInputTokens) || maxInputTokens <= 0) {
		return [
			...
			...messageSections.map((section) => section.text),
	...
	const requiredMessageSections = messageSections.filter(
		(section) => section.index === latestUserMessageIndex,
	);
	const requiredCost = [
		...sectionsBeforeMessages,
		...requiredMessageSections.map((section) => section.text),
		...sectionsAfterMessages,
	].reduce((total, section) => total + getSectionCost(section), 0);
...
export function estimateCursorTextTokens(
	text: string,
	options: Pick<CursorPromptOptions, "charsPerToken"> = {},
): number {
	const charsPerToken = options.charsPerToken ?? CURSOR_APPROX_CHARS_PER_TOKEN;
	return Math.ceil(text.length / charsPerToken);
}

export function estimateCursorPromptTokens(
	prompt: CursorPrompt,
	options: Pick<CursorPromptOptions, "charsPerToken" | "imageTokenEstimate"> = {},
): number {
	return (
		estimateCursorTextTokens(prompt.text, options) +
		prompt.images.length * (options.imageTokenEstimate ?? CURSOR_IMAGE_TOKEN_ESTIMATE)
	);
}

export function estimateCursorPromptMessageTokens(
	message: Message,
	options: Pick<CursorPromptOptions, "charsPerToken"> = {},
): number {
...
export function estimateCursorContextTokens(
	context: Context,
	options: CursorPromptOptions = {},
): number {
	...
	switch (message.role) {
		...
		case "assistant":
			return hashCursorContextValue(
				`assistant:${message.timestamp ?? index}:${JSON.stringify(message.content)}`,
			);
		case "toolResult":
			return hashCursorContextValue(
...
function serializeRawPiMessageForFingerprint(
...
function parseCursorContextFingerprint(
	fingerprint: string,
): CursorContextFingerprintPayload | undefined {
	try {
...
export function computeCursorContextFingerprint(context: Context): string {
	const payload: CursorContextFingerprintPayload = {
		...
		messageHashes: context.messages.map((message, index) =>
			serializeRawPiMessageForFingerprint(message, index),
		),
	};
...
export function buildCursorIncrementalPrompt(
	...
	const latestUserMessage =
		latestUserMessageIndex >= 0 ? messages[latestUserMessageIndex] : undefined;
	...
	const latestUserMessageSections =
		latestUserText && latestUserMessageIndex >= 0
			? [{ index: latestUserMessageIndex, text: latestUserText }]
			: [];
