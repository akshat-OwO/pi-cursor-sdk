...
export function getCursorTaskReplayDescription(
	...
	const fromArgs = typeof args?.description === "string" && args.description.trim() ? args.description.trim() : undefined;
	if (fromArgs) return fromArgs;
	const fromDetails = typeof details?.description === "string" && details.description.trim() ? details.description.trim() : undefined;
	if (fromDetails) return fromDetails;
	const summary = typeof args?.activitySummary === "string" && args.activitySummary.trim() ? args.activitySummary.trim() : undefined;
...
export function renderCursorTaskResult(
	...
	if (options.isPartial) {
		const frame = CURSOR_TASK_SPINNER[spinnerFrame % CURSOR_TASK_SPINNER.length] ?? CURSOR_TASK_SPINNER[0];
	...
	const stats = renderCursorTaskStatsLine(theme, durationMs, isError);
	let line = icon;
	if (description) line += " " + theme.fg("muted", description);
	...
	if (previewSource) {
		const preview = options.expanded ? previewSource.split("\n").slice(0, 50).map((entry) => theme.fg("dim", `  ${entry}`)).join("\n") : formatCursorReplayPreview(previewSource, theme);
		if (preview) line += `\n${preview}`;
	}
	return new Text(line, 0, 0);
}
...
export function renderCursorTaskWidgetRecord(
	...
	return [truncate(`${icon} ${theme.fg("dim", "Task")}  ${theme.fg("dim", record.description)} ${theme.fg("dim", "·")} ${theme.fg("dim", elapsed)}${statusNote}`)];
}
