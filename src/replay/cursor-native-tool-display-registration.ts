...
import { isCursorReplayToolName } from "../transcript/cursor-tool-names.js";

const CORE_PI_TOOL_NAMES = new Set(["read", "bash", "edit", "write"]);

type CursorNativeToolRegistryApi = Pick<
	ExtensionAPI,
...
function syncNativeToolDisplayForModel(
	...
	if (isCursorModel(model)) {
		for (const toolName of registeredNativeToolNames) {
			if (
				...
				!CURSOR_MODEL_ACTIVE_REPLAY_TOOL_NAMES.some(
					(activeReplayToolName) => activeReplayToolName === toolName,
				)
			)
				continue;
...
function registerAvailableNativeCursorTools(
	pi: CursorNativeToolRegistryApi,
	_ctx: NativeRegistrationContext,
): void {
