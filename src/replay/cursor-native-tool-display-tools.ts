...
import { getCursorSessionCwd } from "../session/cursor-session-cwd.js";
import {
...
const CURSOR_REPLAY_TOOL_NAMES = [
	CURSOR_REPLAY_ACTIVITY_TOOL_NAME,
	...CURSOR_REPLAY_LEGACY_TOOL_NAMES,
] as const;
	...
	return {
		...
		async execute(toolCallId, params, signal, onUpdate, ctx) {
			...
			if (isCursorFileMutationToolName(definition.name) && isCursorReplayToolCallId(toolCallId)) {
				throw new Error(
					`No recorded Cursor ${definition.name} result was available. This replay-only call does not execute file mutations.`,
				);
			}
		...
		renderCall(args, theme, context) {
			...
			if (
				...
				isCursorReplayToolCallId(context.toolCallId)
			) {
				return renderNativeLookingCursorFileMutationCall(
					definition.name,
			...
			if (compactDisplay) {
				return renderCompactNativeToolCall(
		...
		renderResult(result, options, theme, context) {
			...
			if (
				isCursorFileMutationToolName(definition.name) &&
				details?.cursorToolName === definition.name
			) {
				if (compactDisplay) {
...
export function createNativeCursorToolDefinition(
	...
	if (toolName === "read")
		return createReadToolDefinition(cwd) as ToolDefinition<TSchema, unknown, unknown>;
	if (toolName === "bash")
		return createBashToolDefinition(cwd) as ToolDefinition<TSchema, unknown, unknown>;
	if (toolName === "edit")
		return createEditToolDefinition(cwd) as ToolDefinition<TSchema, unknown, unknown>;
	if (toolName === "write")
		return createWriteToolDefinition(cwd) as ToolDefinition<TSchema, unknown, unknown>;
	if (toolName === "grep")
		return createGrepToolDefinition(cwd) as ToolDefinition<TSchema, unknown, unknown>;
	if (toolName === "find")
		return createFindToolDefinition(cwd) as ToolDefinition<TSchema, unknown, unknown>;
	if (toolName === "ls")
		return createLsToolDefinition(cwd) as ToolDefinition<TSchema, unknown, unknown>;
	if (isCursorReplayToolName(toolName))
		return createCursorReplayOnlyToolDefinition(toolName) as ToolDefinition<
			TSchema,
			unknown,
			unknown
		>;
...
export function registerNativeCursorTool(
	...
	const registeredTool = wrapNativeCursorTool(definition, () =>
		createNativeCursorToolDefinition(toolName, getCursorSessionCwd()),
	);
	registeredNativeToolDefinitions.set(
		toolName,
		registeredTool as ToolDefinition<TSchema, unknown, unknown>,
	);
