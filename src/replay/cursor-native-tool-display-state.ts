...
import type { CursorPiToolDisplay } from "../transcript/cursor-tool-transcript.js";
import { getCursorCompactToolDisplaySetting } from "../settings/cursor-agent-settings.js";
import { parseOptionalEnvBoolean } from "../shared/cursor-env-boolean.js";
import {
	isCompactCursorReplayToolName,
...
export const registeredNativeToolDefinitions = new Map<
	string,
...
export function isCursorProviderModel(
	model: ExtensionContext["model"] | undefined = currentNativeToolDisplayModel,
): boolean {
...
export function syncCompactNativeToolDisplayShells(): void {
	...
	for (const [toolName, definition] of registeredNativeToolDefinitions) {
		const wantsCompactShell =
			useCompact &&
			(isCompactNativeCursorToolName(toolName) || isCompactCursorReplayToolName(toolName));
...
export function isCursorNativeToolRegistrationRequested(): boolean {
	return (
		readBooleanEnv(NATIVE_CURSOR_TOOL_REGISTRATION_ENV) !== false &&
...
export function consumeCursorNativeToolDisplay(
	id: string,
): CursorNativeToolDisplayItem | undefined {
