...
type CursorReplayRenderCall = NonNullable<
	ToolDefinition<typeof cursorReplayToolSchema, unknown>["renderCall"]
>;
...
function buildImageReplayComponent(
	text: string,
	imageData: string,
	mimeType: string,
	filename: string,
	...
	const imageComponent = new Image(
		...
		{ filename, maxWidthCells: 40, maxHeightCells: 16 },
	);
	return {
...
function truncateCursorReplayLine(
	text: string,
	maxChars = CURSOR_REPLAY_PREVIEW_MAX_LINE_CHARS,
): string {
...
function sliceCursorReplayPreview(
	...
	for (const line of lines) {
		...
		const lineBudget = Math.max(
			Math.min(CURSOR_REPLAY_PREVIEW_MAX_LINE_CHARS, maxChars - usedChars),
			0,
		);
		if (lineBudget <= 0) {
...
function safeHighlightCursorReplayCode(
	text: string,
...
export function formatCursorReplayFilePreview(
	...
	const highlightedLines =
		slice.text.length <= CURSOR_REPLAY_HIGHLIGHT_MAX_CHARS
			? safeHighlightCursorReplayCode(slice.text, path)
			: undefined;
	const renderedLines =
		highlightedLines ?? slice.text.split("\n").map((line) => theme.fg("toolOutput", line));
	const omission = formatCursorReplayOmission(slice);
	if (omission) renderedLines.push(theme.fg("muted", omission));
	return renderedLines.join("\n");
}

function getCursorReplayActivityTitle(
...
function getCursorReplayCallSummary(
	toolName: CursorReplayToolName,
	args: Record<string, unknown> | undefined,
): string | undefined {
...
export function renderNativeLookingCursorFileMutationCall(
	...
	if (toolName === "write" && typeof args?.content === "string" && args.content.length > 0) {
		...
		return new Text(
			`${COMPACT_ROW_PADDING}${formatted}${theme.fg("dim", ` (${pluralize(lineCount, "line")})`)}`,
			0,
			0,
		);
	}
...
function hasCursorEditChanges(details: CursorReplayToolDetails): boolean {
	return (
		Boolean(getCursorEditDiff(details)) ||
		Boolean(details.linesAdded) ||
		Boolean(details.linesRemoved)
	);
}
...
function renderExpandableCursorReplayResult(
	...
	if (expandedText) {
		const preview = options.expanded
			? formatMutedBlock(expandedText, theme)
			: formatCursorReplayPreview(expandedText, theme);
		if (preview) rendered += `\n${preview}`;
	}
	if (details?.cursorToolName === "generateImage" && !isError && context.showImages) {
		...
		if (imageData && mimeType)
			return buildImageReplayComponent(
				rendered,
				imageData,
				mimeType,
				basename(details.imagePath ?? "generated-image"),
				theme,
			);
	}
...
function renderCursorGenerateImageResult(
	...
	return renderExpandableCursorReplayResult(
...
export function renderCursorReplayResult(
	...
	if (details?.cursorToolName === "write") {
		const parts = [
			details.linesCreated !== undefined
				? `${details.linesCreated} line${details.linesCreated === 1 ? "" : "s"}`
				: undefined,
	...
	if (details?.cursorToolName === "generateImage")
		return renderCursorGenerateImageResult(result, options, theme, context, isError);
...
export function createCursorReplayOnlyToolDefinition(
	toolName: CursorReplayToolName,
): ToolDefinition<typeof cursorReplayToolSchema, unknown> {
	const cursorToolName =
		toolName === CURSOR_REPLAY_ACTIVITY_TOOL_NAME
			? "activity"
			: getCursorReplaySourceToolName(toolName);
	...
	return {
		...
		async execute() {
			throw new Error(
				`No recorded Cursor ${cursorToolName} result was available. This replay-only tool does not execute ${sideEffectDescription}.`,
			);
		},
		renderCall(args, theme, context) {
			if (shouldUseCompactCursorReplayToolDisplay(toolName)) {
				return renderCompactCursorReplayCall(
					toolName,
		...
		renderResult(result, options, theme, context) {
			if (shouldUseCompactCursorReplayToolDisplay(toolName)) {
				return renderCompactCursorReplayResult(
					toolName,
					result,
					options,
					theme,
