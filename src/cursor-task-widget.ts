...
type CursorTaskWidgetUi = {
	...
	setWidget(
		key: string,
		content: undefined | ((tui: { terminal: { columns: number }; requestRender(): void }, theme: CursorTaskWidgetTheme) => {
			render(): string[];
			invalidate(): void;
		}),
...
export class CursorTaskWidget {
	...
	update(): void {
		...
		if (!this.widgetRegistered) {
			this.uiCtx.setWidget("cursor-tasks", (tui, theme) => {
				this.tui = tui;
				return {
	...
	private renderWidget(
		...
		const lines: string[] = [
			truncateToWidth(`${theme.fg(running.length > 0 ? "accent" : "dim", running.length > 0 ? "●" : "○")} ${theme.fg(running.length > 0 ? "accent" : "dim", "Cursor tasks")}`, width),
		];
		...
		if (lines.length > maxLines) {
			...
			return [...lines.slice(0, maxLines - 1), truncateToWidth(theme.fg("dim", `└─ +${hidden} more`), width)];
		}
		if (lines.length > 1) {
