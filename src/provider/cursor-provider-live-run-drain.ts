...
} from "../replay/cursor-native-tool-display.js";
import { type CursorPiBridgeToolRequest } from "../bridge/cursor-pi-tool-bridge.js";
import { resetSessionCursorAgent } from "../session/cursor-session-agent.js";
...
import { hasUsableText } from "../shared/cursor-record-utils.js";
import { formatInactiveCursorReplayTrace } from "../replay/cursor-native-replay-trace.js";
import { partitionNativeToolsByActiveContext } from "../replay/cursor-native-replay-routing.js";
...
function trimAlreadyEmittedCursorText(
	text: string,
	emittedText: string,
	...
	if (
		...
		(options?.allowPartialPrefix || isCursorTextBoundary(text, emittedText.length))
	) {
		return text.slice(emittedText.length);
	}
	if (
		emittedText.endsWith(text) &&
		isCursorTextBoundary(emittedText, emittedText.length - text.length)
	)
		return "";
...
function trimCurrentTurnAlreadyEmittedCursorText(
	text: string,
	...
	const currentTurnTrimmedText = trimAlreadyEmittedCursorText(text, currentTurnEmittedText, {
	...
	if (emittedText.endsWith(currentTurnEmittedText)) {
		const emittedTextTrimmedText = trimAlreadyEmittedCursorText(text, emittedText, {
			allowPartialPrefix: true,
		});
...
export function selectCursorFinalText(
	...
	const candidates = [
		typeof resultText === "string" ? resultText : undefined,
		fallbackText,
		textDeltas.join(""),
	];
...
function emitCursorNativeToolUseTurn(
	...
	applyCursorApproximateUsage(
		...
		model,
		context,
		cursorLiveRuns.takeTurnInputTokens(run, toolResultInputTokens),
	);
...
function emitCursorBridgeToolUseTurn(
	...
	for (const request of requests) {
		...
		stream.push({
			type: "toolcall_delta",
			contentIndex,
			delta: JSON.stringify(request.args),
			partial,
		});
		const block = partial.content[contentIndex];
		if (block.type === "toolCall")
...
async function emitCursorLiveRunPendingToolUseTurn(
	...
	if (eventType === "tool") {
		const { active, inactive } = partitionNativeToolsByActiveContext(
			context,
			cursorLiveRuns.collectNativeToolBatch(run),
		);
...
export async function drainCursorLiveRunTurn(
	...
	while (true) {
		...
		if (run.done) {
			...
			const finalText = trimCurrentTurnAlreadyEmittedCursorText(
				run.finalText ?? run.textDeltas.join(""),
				turn.emittedText,
				run.emittedText,
			);
			if (finalText) {
	...
	while (true) {
		...
		try {
			const outcome = await cursorLiveRuns.withRunLease(run, signal, async () => {
				...
				const consumed = cursorLiveRuns.consumeToolResults(
					run,
					context,
					getCursorNativeReplayIdFromToolCallId,
				);
				run.bridgeRun?.resolveToolResults(consumed.toolResults);
				const shouldChainUserInput =
					run.chainUserInputAfterCompletion || hasTrailingUserMessagesAfterToolResults(context);
				...
				if (run.disposed) return "continue_send" as const;
				const drainOutcome = await drainCursorLiveRunTurn(
					stream,
					partial,
					model,
					context,
			...
			if (
				...
				!run.disposed &&
				cursorLiveRuns.getActiveForScope(run.sessionAgentScopeKey) === run
			) {
				continue;
			}
			return outcome;
