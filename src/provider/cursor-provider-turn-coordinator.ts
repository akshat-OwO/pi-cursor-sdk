...
import { formatInactiveCursorReplayTrace } from "../replay/cursor-native-replay-trace.js";
import { resolveNativeReplayDisposition } from "../replay/cursor-native-replay-routing.js";
import { truncateCursorDisplayLine } from "../shared/cursor-display-text.js";
import { CursorPartialContentEmitter } from "./cursor-partial-content-emitter.js";
import { asRecord, getField, hasUsableText } from "../shared/cursor-record-utils.js";
import { scrubPiToolDisplay, scrubSensitiveText } from "../shared/cursor-sensitive-text.js";
import {
	buildCursorPiToolDisplay,
	formatCursorToolTranscript,
	getCursorCreatePlanText,
	mergeCursorToolCalls,
} from "../transcript/cursor-tool-transcript.js";
import {
	completeCursorTask,
	...
	startCursorTask,
} from "../task/cursor-task-ui.js";
import { getToolName } from "../transcript/cursor-transcript-utils.js";
...
function isCursorShellToolCall(toolCall: unknown): boolean {
	...
	return (
		...
		normalizedName === "run_terminal_cmd" ||
...
function mergeShellOutputDeltasIntoCursorToolCall(
	toolCall: unknown,
...
export class CursorSdkTurnCoordinator {
	...
	constructor(options: CursorSdkTurnCoordinatorOptions) {
		...
		this.contentEmitter = new CursorPartialContentEmitter(
			options.stream,
			options.partial,
			undefined,
			false,
	...
	handleStep(stepEnvelope: unknown): void {
		...
		const stepId =
			getField(stepEnvelope, "id") ?? getField(toolCall, "id") ?? getField(toolCall, "callId");
		if (!toolCall) return;
	...
	private resolveToolCompletion(options: {
		...
		if (this.liveRun?.bridgeRun?.isBridgeMcpToolCall(resolvedToolCall)) {
			const bridgeIdentity =
				options.source === "step" && matchedStartedCallId
	...
	private handleCompletedToolCall(
		...
		const transcript = scrubSensitiveText(
			formatCursorToolTranscript(toolCall, { cwd: this.cwd }),
			this.resolvedApiKey,
		);
		const display = buildCursorPiToolDisplay(toolCall, { cwd: this.cwd });
		this.trackCursorTaskCompletion(toolCall, display, options.identity);
		const fingerprint = this.getToolFingerprint({
		...
		if (options.source === "started") {
			...
			this.completedStartedToolFingerprints.has(fingerprint) ||
			this.completedFallbackToolFingerprints.has(fingerprint)
		) {
			return;
		}
	...
	private trackCursorTaskCompletion(
		toolCall: unknown,
		display: ReturnType<typeof buildCursorPiToolDisplay>,
		identity?: string,
	): void {
		if (!isCursorTaskToolCall(toolCall)) return;
		const callId = identity?.startsWith("cursor-tool:")
			? identity.slice("cursor-tool:".length)
			: undefined;
		if (!callId) return;
		const summary = display.result.content
			.map((entry) => (entry.type === "text" ? entry.text : undefined))
			.filter((entry): entry is string => Boolean(entry))
			.join("\n")
			.split("\n")
			.find((line) => line.trim());
		completeCursorTask(callId, display.isError ? "error" : "completed", summary);
		const task = getCursorTask(callId);
		if (
			task?.completedAt !== undefined &&
			display.result.details &&
			typeof display.result.details === "object"
		) {
			(display.result.details as Record<string, unknown>).durationMs =
	...
	private maybeEmitCursorTaskProgress(callId: unknown, toolCall: unknown): void {
		...
		const label = extractCursorTaskDescription(toolCall, this.resolvedApiKey);
		if (!label) return;
		this.emittedTaskProgressCallIds.add(callId);
		startCursorTask(callId, label);
		if (shouldSuppressCursorTaskThinkingProgress()) return;
	...
	private getStartedToolCallFingerprint(toolCall: unknown): string {
		return this.getToolFingerprint({
			toolName: getToolName(toolCall),
			args: getField(toolCall, "args"),
		});
	}
