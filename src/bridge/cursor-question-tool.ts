...
interface CursorQuestionToolExtensionApi extends Pick<
	ExtensionAPI,
...
> {
	...
	on(
		event: "model_select",
		handler: (
			event: { model: ExtensionContext["model"] },
...
const QuestionOptionSchema = Type.Union([
	...
	Type.Object({
		...
		value: Type.Optional(
			Type.String({ description: "Optional value returned to Cursor; defaults to label" }),
		),
		description: Type.Optional(
			Type.String({ description: "Optional helper text shown by compatible pi UIs" }),
		),
	}),
]);
...
const QuestionSchema = Type.Object({
	...
	options: Type.Optional(
		Type.Array(QuestionOptionSchema, { description: "Choices the user can select" }),
	),
	choices: Type.Optional(Type.Array(QuestionOptionSchema, { description: "Alias for options" })),
	allowCustom: Type.Optional(
		Type.Boolean({
			description: "Allow a typed answer in addition to listed options; defaults to true",
		}),
	),
});
...
const CursorAskQuestionParamsSchema = Type.Object({
	...
	questions: Type.Optional(
		Type.Array(QuestionSchema, { description: "Ask multiple questions sequentially" }),
	),
});
...
function normalizeOption(
	...
	index: number,
): CursorQuestionOption | undefined {
...
function normalizeOptions(options: RawQuestionOption[] | undefined): CursorQuestionOption[] {
	return (options ?? [])
		.map(normalizeOption)
		.filter((option): option is CursorQuestionOption => option !== undefined);
}
...
function normalizeQuestions(params: CursorAskQuestionParams): CursorQuestion[] {
	const rawQuestions =
		Array.isArray(params.questions) && params.questions.length > 0 ? params.questions : [params];
	return rawQuestions
		.map(normalizeQuestion)
		.filter((question): question is CursorQuestion => question !== undefined);
}
...
function summarizeAnswers(answers: CursorQuestionAnswer[]): string {
	...
	if (answers.length === 1) {
		...
		return answer.cancelled || answer.answer === null
...
function syncCursorQuestionToolForModel(
...
export function registerCursorQuestionTool(pi: CursorQuestionToolExtensionApi): void {
	pi.registerTool({
		...
		renderCall(args, theme) {
			...
			return new Text(
				theme.fg("toolTitle", theme.bold("cursor question ")) + theme.fg("muted", label),
				0,
				0,
			);
		},
