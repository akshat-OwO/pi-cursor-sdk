...
export function resolveCursorPiToolBridgeDebugEnabled(
...
export function serializeCursorPiToolBridgeDiagnostic(
		...
		case "tools_exposed":
			return {
				...
				pairs: event.pairs.map((pair) => ({
					piToolName: pair.piToolName,
					mcpToolName: pair.mcpToolName,
				})),
			};
		case "run_cancelled":
			return {
...
export function writeCursorPiToolBridgeDiagnostic(
	...
	try {
		process.stderr.write(
			`${CURSOR_PI_TOOL_BRIDGE_DIAGNOSTIC_PREFIX} ${JSON.stringify(serializeCursorPiToolBridgeDiagnostic(event))}\n`,
		);
	} catch {
