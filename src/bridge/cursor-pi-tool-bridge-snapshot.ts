...
import { parseEnvBoolean } from "../shared/cursor-env-boolean.js";
import {
	createMcpToolName,
	normalizeMcpInputSchema,
	stableNameHash,
} from "./cursor-pi-tool-bridge-mcp.js";
import { isRegisteredCursorNativeToolName } from "../replay/cursor-native-tool-display-state.js";
import { isExcludedFromCursorBridgeExposure } from "../transcript/cursor-tool-names.js";
...
const OVERLAPPING_CURSOR_NATIVE_PI_BUILTIN_TOOL_NAMES = new Set([
	"read",
	"bash",
	"write",
	"edit",
...
export function resolveCursorPiToolBridgeEnabled(
	env: Record<string, string | undefined> = process.env,
): boolean {
	return parseEnvBoolean(env[CURSOR_PI_TOOL_BRIDGE_ENV], true);
}

export function resolveCursorPiToolBridgeBuiltinsEnabled(
	env: Record<string, string | undefined> = process.env,
): boolean {
...
export function buildCursorPiToolBridgeSurfaceSignature(
...
export function buildCursorPiToolBridgeSnapshot(
	...
	for (const tool of allTools) {
		...
		if (
			isExcludedFromCursorBridgeExposure(tool.name) &&
			isRegisteredCursorNativeToolName(tool.name)
		)
			continue;
