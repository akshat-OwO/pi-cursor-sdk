...
export class CursorPiToolBridgeRunImpl implements CursorPiToolBridgeRun {
	...
	async handleHttpRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
		if (this.disposed || !this.mcpTransport) {
			res
				...
				.end(JSON.stringify({ error: "Cursor pi tool bridge run is disposed" }));
			return;
		}
	...
	resolveToolResultsFromContext(context: Context): void {
		this.resolveToolResults(
			context.messages
				.map(asToolResultMessage)
				.filter((message): message is ToolResultMessage => message !== undefined),
		);
	}
	...
	private enqueueToolRequest(
		...
		return new Promise<CallToolResult>((resolve, reject) => {
			...
			pending.onAbort = () => {
				this.rejectPending(
					pending,
					new Error("Cursor MCP bridge tool request was aborted"),
					"cancelled",
				);
			};
			if (signal?.aborted) {
			...
			if (!this.onToolRequest) {
				if (this.liveRunHandlerDetached) {
					this.rejectPending(
						pending,
						new Error("Cursor pi tool bridge has no active live run"),
						"cancelled",
					);
					return;
				}
	...
	private lifecycleDiagnosticFields(
		pendingCount = this.pendingCount(),
	): CursorPiToolBridgeLifecycleDiagnosticFields {
		return {
			runId: this.id,
	...
	private requestDiagnosticFields(
		request: CursorPiBridgeToolRequest,
	): CursorPiToolBridgeRequestDiagnosticFields {
		return {
			runId: this.id,
	...
	private emitRequestResolvedDiagnostic(
		request: CursorPiBridgeToolRequest,
		isError: boolean,
	): void {
		this.emitDiagnostic({
			event: "request_resolved",
			...this.requestDiagnosticFields(request),
			isError,
		});
	}

	private emitRequestRejectedDiagnostic(
		request: CursorPiBridgeToolRequest,
	...
	private removePending(pending: PendingBridgeCall): void {
		...
		const queuedIndex = this.queuedRequests.findIndex(
			(request) => request.bridgeCallId === pending.request.bridgeCallId,
		);
