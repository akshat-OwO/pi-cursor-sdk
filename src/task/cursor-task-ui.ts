...
import { getString, getToolArgs, getToolName } from "../transcript/cursor-transcript-utils.js";
...
export function isCursorTaskWidgetEnabled(
	env: Record<string, string | undefined> = process.env,
): boolean {
	return parseEnvBoolean(env[CURSOR_TASK_WIDGET_ENV], true);
}

export function isCursorTaskDisplayEnabled(
	env: Record<string, string | undefined> = process.env,
): boolean {
...
export function shouldSuppressCursorTaskThinkingProgress(
	env: Record<string, string | undefined> = process.env,
): boolean {
...
export function extractCursorTaskDescription(
	toolCall: unknown,
	apiKey?: string,
): string | undefined {
...
export function completeCursorTask(
	callId: string,
	status: Exclude<CursorTaskStatus, "running">,
	activity?: string,
): void {
	const existing = cursorTasks.get(callId);
	if (!existing) return;
