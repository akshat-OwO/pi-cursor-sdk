...
function buildGenericPiToolDisplay(context: ToolDisplayContext): CursorPiToolDisplay {
	...
	return {
		...
		result: textToolResult(
			isError ? formatError(result.error) : limitText(stringifyUnknown(result.value), options),
		),
		isError,
	};
}
...
const TOOL_DISPLAY_SPECS: Record<string, ToolDisplaySpec> = {
			...
			return {
				...
				result: textToolResult(
					isError
						...
						: formatNativeReadDisplayContent(args, result, options),
				),
				isError,
			};
		},
	...
	shell: {
		...
		buildPiToolDisplay: ({ args, result, options }) => {
			...
			const isError =
				...
				(shellOutput.exitCode !== undefined && shellOutput.exitCode !== 0);
			return {
				toolName: "bash",
				...
				result: textToolResult(
					result.status === "error"
						? formatError(result.error)
						: limitText(shellOutput.text, options),
				),
				isError,
			};
		},
	...
	ls: {
		...
		buildPiToolDisplay: ({ args, result, options }) => ({
			...
			args,
			result: textToolResult(
				result.status === "error" ? formatError(result.error) : getLsBody(result, options).trim(),
			),
			isError: result.status === "error",
		}),
	},
	...
	delete: {
		...
		buildPiToolDisplay: (context) =>
			buildActivityReplayDisplay("delete", TOOL_DISPLAY_SPECS.delete, context),
		activityReplay: {
	...
	readLints: {
		...
		buildPiToolDisplay: (context) =>
			buildActivityReplayDisplay("readLints", TOOL_DISPLAY_SPECS.readLints, context),
		activityReplay: {
	...
	updateTodos: {
		formatTranscript: ({ args, result, options }) =>
			formatTodos(args, result, options, "updateTodos"),
		buildPiToolDisplay: (context) =>
			buildActivityReplayDisplay("updateTodos", TOOL_DISPLAY_SPECS.updateTodos, context),
		activityReplay: {
	...
	createPlan: {
		...
		buildPiToolDisplay: (context) =>
			buildActivityReplayDisplay("createPlan", TOOL_DISPLAY_SPECS.createPlan, context),
		activityReplay: {
	...
	task: {
		...
		buildPiToolDisplay: (context) =>
			buildActivityReplayDisplay("task", TOOL_DISPLAY_SPECS.task, context),
		activityReplay: {
			...
			buildActivityArgs: ({ args, result }) => {
				...
				return {
					...
					activitySummary: truncateArg(summarizeTask(description, collectTaskText(result))),
				};
			},
	...
	generateImage: {
		...
		buildPiToolDisplay: (context) =>
			buildActivityReplayDisplay("generateImage", TOOL_DISPLAY_SPECS.generateImage, context),
		activityReplay: {
	...
	mcp: {
		...
		buildPiToolDisplay: (context) =>
			buildActivityReplayDisplay("mcp", TOOL_DISPLAY_SPECS.mcp, context),
		activityReplay: {
			labelKey: "cursor_mcp",
			buildActivityArgs: ({ args }) => {
			...
			buildDetails: ({ result }, contentText) => ({
				summary:
					result.status === "error"
						? undefined
						: (firstNonEmptyLine(contentText) ?? "MCP result captured"),
			}),
		},
	},
};
