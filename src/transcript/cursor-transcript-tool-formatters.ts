import { resolveCursorEditDiff } from "../replay/cursor-edit-diff.js";
import { getFirstStringByKeys } from "../shared/cursor-record-utils.js";
import {
	asRecord,
	formatDisplayPath,
	formatDiffString,
...
export function formatRead(
	...
	return joinSections(
		`read ${path}`,
		limitText(getReadContent(args, result, options), readOptions, totalLines),
	);
}
...
export function buildWriteDisplayArgs(
	args: Record<string, unknown>,
	options: TranscriptOptions,
): Record<string, unknown> {
...
const CURSOR_EDIT_OLD_TEXT_KEYS = [
	"oldText",
	"old_text",
	"oldString",
	"old_string",
...
const CURSOR_EDIT_NEW_TEXT_KEYS = [
	"newText",
	"new_text",
	"newString",
	"new_string",
...
const CURSOR_NOTEBOOK_EDIT_ARG_KEYS = [
	"cellId",
	"cell_id",
	"cellIndex",
	"cell_index",
...
function getNativeEditReplacementsFromArgs(
	args: Record<string, unknown>,
): NativeEditReplacement[] | undefined {
	const edits = getArray(args, "edits")?.map(asNativeEditReplacement);
	if (
		edits &&
		edits.length > 0 &&
		edits.every((edit): edit is NativeEditReplacement => edit !== undefined)
	)
		return edits;
...
export function buildNativeEditDisplayArgs(
	rawToolName: string,
	args: Record<string, unknown>,
	options: TranscriptOptions,
): NativeEditDisplayArgs | undefined {
...
export function buildCursorEditActivityDisplayArgs(
	args: Record<string, unknown>,
	options: TranscriptOptions,
): Record<string, unknown> {
	const rawPath = getCursorEditPathArg(args);
...
export function formatNativeReadDisplayContent(
	args: Record<string, unknown>,
	result: NormalizedResult,
	options: TranscriptOptions,
): string {
	...
	if (visible.length > (readOptions.maxChars ?? DEFAULT_READ_TRANSCRIPT_CHARS))
		return limitText(content, readOptions, totalLines);
...
export function getShellOutput(
	result: NormalizedResult,
	args: Record<string, unknown> = {},
): { text: string; exitCode: number | undefined; timedOut: boolean } {
	const value = asRecord(result.value);
	...
	const timedOut =
		timeoutMs !== undefined && executionTimeMs !== undefined && executionTimeMs >= timeoutMs;
	const outputParts: string[] = [];
	if (stdout) outputParts.push(stdout.trimEnd());
	if (stderr) outputParts.push(stderr.trimEnd());
	if (exitCode !== undefined && exitCode !== 0)
		outputParts.push(`Command exited with code ${exitCode}`);
	if (timedOut)
		outputParts.push(`Command backgrounded after ${(timeoutMs / 1000).toFixed(0)} second timeout`);
...
export function formatShell(
	...
	if (result.status === "error")
		return joinSections(`$ ${command || "shell"}`, formatError(result.error));
	...
	return joinSections(
		`$ ${command || "shell"}`,
		limitText(outputParts.filter(Boolean).join("\n\n"), options),
	);
}
...
function renderTreeNode(node: unknown, depth = 0, lines: string[] = []): string[] {
	...
	const children =
		getArray(record, "children") ?? getArray(record, "entries") ?? getArray(record, "files") ?? [];
...
function collectSearchResults(value: unknown): string[] {
	...
	for (const outputValue of outputs) {
		...
		if (type === "content") {
			...
			if (matches.length === 0 && getNumber(output, "totalMatches") === 0)
			...
			if (files.length === 0 && getNumber(output, "totalMatches") === 0)
				sawExplicitNoMatches = true;
			lines.push(
				...files
					.filter((entry): entry is string => typeof entry === "string")
					.map(formatSearchFile),
			);
		} else if (type === "count") {
			const counts = getArray(output, "counts") ?? [];
			if (counts.length === 0 && getNumber(output, "totalMatches") === 0)
				sawExplicitNoMatches = true;
			for (const count of counts) {
				const countRecord = asRecord(count);
				lines.push(
					`${getString(countRecord, "file") ?? ""}: ${getNumber(countRecord, "count") ?? 0}`.trim(),
				);
			}
		} else {
	...
	if (lines.length === 0 && topLevelTotalMatches !== undefined) {
		return topLevelTotalMatches === 0
			? ["(no matches)"]
			: [formatSearchCount(topLevelTotalMatches)];
	}
	if (lines.length === 0 && sawExplicitNoMatches) return ["(no matches)"];
	return lines.filter(Boolean);
}

function synthesizeGrepBashCommand(
	args: Record<string, unknown>,
	options: TranscriptOptions,
): string {
	const pattern = typeof args.pattern === "string" ? args.pattern : "";
	const path =
		typeof args.path === "string" ? formatDisplayPath(args.path, options.cwd) : undefined;
	const glob = typeof args.glob === "string" ? args.glob : undefined;
	return ["grep", pattern && JSON.stringify(pattern), path ?? glob].filter(Boolean).join(" ");
}

export function buildGrepDisplayArgs(
	args: Record<string, unknown>,
	options: TranscriptOptions,
): Record<string, unknown> {
	...
	const context =
		getNumber(args, "context") ??
		getNumber(args, "contextBefore") ??
		getNumber(args, "contextAfter");
...
function getGlobPattern(args: Record<string, unknown>): string {
	return typeof args.globPattern === "string"
		? args.globPattern
...
function getGlobTargetDirectory(
	args: Record<string, unknown>,
	options: TranscriptOptions,
): string | undefined {
	const rawPath =
...
function synthesizeGlobBashCommand(
	args: Record<string, unknown>,
	options: TranscriptOptions,
): string {
...
export function buildFindDisplayArgs(
	args: Record<string, unknown>,
	options: TranscriptOptions,
): Record<string, unknown> {
...
export function getGrepBody(result: NormalizedResult, options: TranscriptOptions): string {
	...
	const body =
		limited.omitted > 0
			? `${limited.items.join("\n")}\n... (${limited.omitted} more matches truncated)`
			: limited.items.join("\n");
...
export function getGlobBody(result: NormalizedResult, options: TranscriptOptions): string {
	const value = asRecord(result.value);
	const files =
		getArray(value, "files")?.filter((entry): entry is string => typeof entry === "string") ?? [];
	if (files.length === 0) {
...
export function getCursorWriteArgContent(args: Record<string, unknown>): string | undefined {
	return (
		getString(args, "content") ?? getString(args, "fileContent") ?? getString(args, "contents")
	);
}

function getCursorWriteRecordedContent(
	args: Record<string, unknown>,
...
export function formatEdit(
	...
	const stats = [
		...
		.filter(Boolean)
		.join(" ");
	const body = [stats, diff ? limitText(diff, options) : undefined]
		.filter((part): part is string => Boolean(part))
		.join("\n\n");
...
export function formatDelete(
	...
	return joinSections(
		`delete ${path}`,
		fileSize !== undefined ? `Deleted ${fileSize} bytes` : stringifyUnknown(result.value),
	);
}
...
export function getReadLintPaths(
	...
	return [
		...new Set(
			[...explicitPaths, ...resultPaths].map((entry) => formatDisplayPath(entry, options.cwd)),
		),
	];
}

export function getReadLintDiagnostics(
	result: NormalizedResult,
	options: TranscriptOptions,
): string[] {
	const value = asRecord(result.value);
...
export function formatReadLints(
	args: Record<string, unknown>,
	result: NormalizedResult,
	options: TranscriptOptions,
): string {
	...
	if (lines.length === 0 && paths.length > 0)
		return joinSections(header, `No diagnostics in ${paths.join(", ")}`);
	return joinSections(
		header,
		limitText(lines.join("\n") || stringifyUnknown(result.value), options),
	);
}
...
export function getTodoTotalCount(
	args: Record<string, unknown>,
	result: NormalizedResult,
	todos: Array<{ content: string; status?: string }>,
): number {
	return (
		getNumber(asRecord(result.value), "totalCount") ?? getNumber(args, "totalCount") ?? todos.length
	);
}
...
export function formatTodos(
	...
	const todos = getTodoItems(args, result);
	if (todos.length === 0)
		return joinSections(header, limitText(stringifyUnknown(result.value), options));
	const lines = todos.map(
		(todo) =>
			`${formatTodoStatus(todo.status)} ${todo.content}${todo.status ? ` (${todo.status})` : ""}`,
	);
...
export function getTaskDescription(
	...
	return (
		getString(args, "description") ?? getString(asRecord(result.value), "description") ?? "task"
	);
}
...
export function formatTask(
	...
	if (result.status === "error")
		return joinSections(`task ${description}`, formatError(result.error));
	const taskText = collectTaskText(result);
	return joinSections(
		`task ${description}`,
		limitText(taskText || stringifyUnknown(result.value), options),
	);
}
...
export function getGenerateImagePath(
	args: Record<string, unknown>,
	result: NormalizedResult,
): string | undefined {
...
export function formatGenerateImage(
	args: Record<string, unknown>,
	result: NormalizedResult,
	options: TranscriptOptions,
): string {
	...
	if (result.status === "error")
		return joinSections(`generateImage ${prompt}`, formatError(result.error));
	const value = getGenerateImageValue(result);
	...
	const lines = [
		displayPath ? `Saved image: ${displayPath}` : undefined,
		hasImageData ? "Image data returned by Cursor SDK." : undefined,
	].filter((line): line is string => Boolean(line));
	if (lines.length > 0) return joinSections(`generateImage ${prompt}`, lines.join("\n"));
	return joinSections(
		`generateImage ${prompt}`,
		limitText(stringifyUnknown(result.value), options),
	);
}
...
function describeNonTextMcpContent(entry: unknown): string {
	...
	if (type === "image") {
		const mimeType =
			getString(record, "mimeType") ?? getString(record, "mime") ?? getString(record, "mediaType");
...
export function formatMcp(
	...
	const contentSummary =
		content.length > 0
			? content.map(describeNonTextMcpContent).join("\n")
			: stringifyUnknown(result.value);
...
export function formatFallback(
	...
	return joinSections(
		header,
		limitText(`${argsText}${stringifyUnknown(result.value)}`.trim(), options),
	);
}
