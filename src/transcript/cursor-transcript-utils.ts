...
import { asRecord } from "../shared/cursor-record-utils.js";

export { asRecord, getFirstStringByKeys } from "../shared/cursor-record-utils.js";
...
export function limitText(
	...
	if (limited.length > maxChars) {
		...
		truncatedLines = Math.max(
			truncatedLines,
			Math.max((knownTotalLines ?? lines.length) - limitedLines.length, 0),
		);
	}
...
export function formatDiffString(
...
export function readFilePreview(path: string, options: TranscriptOptions): string | undefined {
	...
	try {
		...
		if (
			!isPathWithinCwd(realFilePath, realCwd) ||
			isSensitivePreviewPath(filePath) ||
			isSensitivePreviewPath(realFilePath)
		)
			return undefined;
