...
function getCompactCursorReplayActivityTitle(toolName: CursorReplayToolName, args: Record<string, unknown>): string {
...
function getCompactCursorReplayCallSummary(toolName: CursorReplayToolName, args: Record<string, unknown>): string | undefined {
...
function withReplayMutationArgs(
	...
	if (typeof details?.path === "string" && details.path.length > 0) return { ...record, path: details.path };
	return record;
}
...
function renderCompactFileMutationToolResult(
	...
	const previewLines = buildCompactFileMutationPreviewLines(toolName, result, mutationArgs, theme, options.expanded ?? false);
	if (previewLines) {
		const callLine = callLineOverride ?? buildCompactFileMutationCallLine(toolName, mutationArgs, theme, context.cwd);
...
export function renderCompactFileMutationResult(
	...
	return renderCompactFileMutationToolResult(toolName, result, options, theme, context, isError, getExpandedRenderResult);
}
...
function getCompactCursorReplayMutationToolName(
	...
	const details = asCursorReplayToolDetails(result.details);
	if (details?.cursorToolName === "edit" || details?.cursorToolName === "write") return details.cursorToolName;
	return undefined;
}
...
export function countCompactSearchMatches(toolName: "grep" | "find", result: CompactToolResult): number {
...
export function renderCompactCursorReplayCall(
	...
	const text = asTextComponent(context.lastComponent);
	text.setText(withCompactPadding(formatCompactCursorReplayCall(toolName, args, theme, context.cwd)));
	return text;
}
...
export function renderCompactCursorReplayResult(
	...
	if (mutationToolName) {
		...
		if (mutationToolName === "write" && typeof mutationArgs.content === "string" && mutationArgs.content.length > 0) {
