...
function resolveCompactFileMutationDiff(
	...
	const recordedDiff = resolveCursorEditDiff(details) ?? resolveCursorEditDiff(result.details) ?? getNativeEditDiff(result.details);
...
export function buildCompactFileMutationPreviewLines(
	...
	if (diff) return buildCompactDiffPreviewLines(diff, theme, maxLines, path !== "unknown" ? path : undefined);
	...
	if (text && !details?.cursorToolName) {
		...
		return buildCompactDiffPreviewLines(synthetic, theme, maxLines, path !== "unknown" ? path : undefined);
	}
	return undefined;
}
...
export function buildCompactFileMutationPreviewText(
	...
	const previewLines = buildCompactFileMutationPreviewLines(toolName, result, args, theme, expanded);
