...
describe("streamCursor native replay post-tool text", () => {
	...
	it("streams post-tool Cursor thinking and text while a native replay run is still active", async () => {
		...
		await Promise.race([
			...
			new Promise((_, reject) =>
				setTimeout(() => reject(new Error("timed out waiting for live Cursor text")), 500),
			),
		]);
	...
	it("queues post-tool thinking and text that arrive before the native tool-use turn closes", async () => {
		...
		await Promise.race([
			liveTextSeen,
			new Promise((_, reject) =>
				setTimeout(() => reject(new Error("timed out waiting for queued Cursor text")), 500),
			),
		]);
		resolveRun({ id: "run-1", status: "finished", result: "Final answer." });
		await replayDone;
	...
	it("does not duplicate text already emitted before a later native replay tool", async () => {
		...
		const readTool = registeredTools.find((tool) => tool.name === "read");
		const firstToolResult = await readTool.execute(
			firstToolCall.id,
		...
		onDelta?.({
			update: {
				type: "tool-call-started",
				toolCall: { name: "grep", args: { pattern: "cursor", path: "src" } },
				callId: "c2",
			},
		});
		onDelta?.({
		...
		await Promise.race([
			secondToolSeen,
			new Promise((_, reject) =>
				setTimeout(() => reject(new Error("timed out waiting for second replay tool")), 500),
			),
		]);
		...
		const secondToolCall = getDoneEvent(secondEvents).message.content.find(isToolCallBlock);
		const grepTool = registeredTools.find((tool) => tool.name === "grep");
		const secondToolResult = await grepTool.execute(
			secondToolCall.id,
			secondToolCall.arguments,
			undefined,
			undefined,
			{},
		);
		...
		const finalEventsPromise = collectEvents(
			streamCursor(makeModel(), finalContext, { apiKey: "test-key" }),
		);
		await Promise.resolve();
	...
	it("does not duplicate final result after an earlier post-tool text turn", async () => {
		...
		const firstEvents = await collectEvents(
			streamCursor(makeModel(), context, { apiKey: "test-key" }),
		);
		const firstDone = getDoneEvent(firstEvents);
		const firstToolCall = firstDone.message.content.find(isToolCallBlock);
		const firstToolResult = await readTool.execute(
		...
		onDelta?.({
			update: {
				type: "tool-call-started",
				toolCall: { name: "read", args: { path: "src/index.ts" } },
				callId: "c2",
			},
		});
		onDelta?.({
		...
		const secondToolResult = await readTool.execute(
			secondToolCall.id,
			secondToolCall.arguments,
			undefined,
			undefined,
		...
		expect(firstDone.message.usage.totalTokens).toBeGreaterThan(
			firstDone.message.usage.input + firstDone.message.usage.output,
		);
		expect(secondDone.message.usage.input).toBe(
			estimateCursorPromptMessageTokens(firstToolResultMessage),
		);
		...
		expect(secondDone.message.usage.totalTokens).toBeGreaterThan(
			secondDone.message.usage.input + secondDone.message.usage.output,
		);
		expect(finalDone.message.usage.input).toBe(
			estimateCursorPromptMessageTokens(secondToolResultMessage),
		);
		expect(finalDone.message.usage.input).not.toBe(
			estimateCursorPromptMessageTokens(firstToolResultMessage) +
				estimateCursorPromptMessageTokens(secondToolResultMessage),
		);
		...
		expect(finalDone.message.usage.totalTokens).toBeGreaterThan(
			finalDone.message.usage.input + finalDone.message.usage.output,
		);
	...
	it("does not trim final text when pre-tool text is only a word prefix", async () => {
		...
		const firstEventsPromise = collectEvents(
		...
		try {
			...
			resolveRun({
				id: "run-1",
				status: "finished",
				result: "Disconnecting the CDP session per your choice.",
			});
			const finalEvents = await finalEventsPromise;
			const finalText = collectTextDeltas(finalEvents);
			const finalDone = getDoneEvent(finalEvents);
			...
			expect(finalDone.message.content).toEqual([
				{ type: "text", text: "Disconnecting the CDP session per your choice." },
			]);
		} finally {
