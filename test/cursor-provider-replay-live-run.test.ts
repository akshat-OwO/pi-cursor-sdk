...
describe("streamCursor native replay live run", () => {
	...
	it("replays native Cursor tools as a toolUse turn before final text", async () => {
		...
		const replayEvents = await collectEvents(
			streamCursor(makeModel(), replayContext, { apiKey: "test-key" }),
		);
		...
		const mockSend = vi
			...
			.mockImplementation(async (_msg: unknown, opts: { onDelta: CursorDeltaHandler }) => {
				...
				return {
					...
					wait: vi
						.fn()
						.mockResolvedValue({ id: "run-1", status: "finished", result: "Task complete." }),
					cancel: vi.fn(),
					supports: () => true,
					unsupportedReason: () => undefined,
				};
	...
	it("does not replay queued live-run tools that became inactive after the run started", async () => {
			...
			.mockImplementation(async (_msg: unknown, opts: { onDelta: CursorDeltaHandler }) => {
				...
				opts.onDelta({
					update: {
						...
						toolCall: {
							name: "read",
							result: { status: "success", value: { content: "# Project" } },
						},
						callId: "read-1",
					},
				});
		...
		firstOnDelta?.({
			update: {
				type: "tool-call-started",
				toolCall: { name: "grep", args: { pattern: "metric-link", path: "src/app/globals.css" } },
				callId: "grep-1",
			},
		});
		firstOnDelta?.({
			update: {
				...
				toolCall: {
					name: "grep",
					result: { status: "success", value: { matches: ["src/app/globals.css"] } },
				},
				callId: "grep-1",
			},
		});
	...
	it("resumes an active live run when a steering user message follows tool results", async () => {
		...
		const mockSend = vi
			.fn()
			.mockImplementation(
				async (message: { text?: string }, opts: { onDelta: CursorDeltaHandler }) => {
					sendCallCount += 1;
					if (sendCallCount === 1) {
						firstOnDelta = opts.onDelta;
						opts.onDelta({
						...
						opts.onDelta({
							update: {
								...
								toolCall: {
									...
									result: {
										status: "success",
										value: { stdout: "clean", stderr: "", exitCode: 0 },
									},
								},
								callId: "c1",
							},
					...
					return {
						...
						wait: vi
							.fn()
							.mockResolvedValue({ id: "run-2", status: "finished", result: message.text ?? "" }),
						cancel: vi.fn(),
						supports: () => true,
						unsupportedReason: () => undefined,
					};
		...
		const steerEventsPromise = collectEvents(
			streamCursor(makeModel(), steerContext, { apiKey: "test-key" }),
		);
	...
	it("settles a scope-active live run directly when context has no matching tool results", async () => {
		...
		const scopedEventsPromise = collectEvents(
			streamCursor(makeModel(), makeContext(), { apiKey: "test-key" }),
		);
		...
		const scopedEvents = await Promise.race([
			...
			new Promise<never>((_, reject) =>
				setTimeout(() => reject(new Error("scope-active live run settlement timed out")), 1000),
			),
		]);
	...
	it("drops additional old-run tool batches when steering user input should start a fresh send", async () => {
		...
		const mockSend = vi
			.fn()
			.mockImplementation(
				async (_message: { text?: string }, opts: { onDelta: CursorDeltaHandler }) => {
					sendCallCount += 1;
					if (sendCallCount === 1) {
						firstOnDelta = opts.onDelta;
						opts.onDelta({
		...
		const bashTool = registeredTools.find((tool) => tool.name === "bash");
		const firstToolResult = await bashTool!.execute(
			firstToolCall!.id,
		...
		firstOnDelta?.({
			update: {
				type: "tool-call-started",
				toolCall: { name: "bash", args: { command: "git log -1" } },
				callId: "c2",
			},
		});
		firstOnDelta?.({
