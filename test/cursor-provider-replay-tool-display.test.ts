...
describe("streamCursor native replay tool display", () => {
			...
			const mockSend = vi
				...
				.mockImplementation(async (_msg: unknown, opts: { onDelta: CursorDeltaHandler }) => {
					opts.onDelta({
						update: {
							type: "tool-call-started",
							toolCall: { type: "edit", args: { path: targetPath } },
							callId: "c1",
						},
					});
					opts.onDelta({
			...
			const mockSend = vi
				...
				.mockImplementation(async (_msg: unknown, opts: { onDelta: CursorDeltaHandler }) => {
					opts.onDelta({
						update: {
							type: "tool-call-started",
							toolCall: { type: "write", args: { path: targetPath } },
							callId: "c1",
						},
					});
					opts.onDelta({
	...
	it("replays Cursor write activity through native-looking recorded write output without mutating files", async () => {
		...
		try {
			...
			const mockSend = vi
				...
				.mockImplementation(async (_msg: unknown, opts: { onDelta: CursorDeltaHandler }) => {
					opts.onDelta({
						update: {
							type: "tool-call-started",
							toolCall: { type: "write", args: { path: targetPath, content: "new\n" } },
							callId: "c1",
						},
					});
					opts.onDelta({
			...
			await expect(
				writeTool!.execute(
					"cursor-replay-1-1-tool-998",
					{ path: targetPath, content: "mutated\n" },
					undefined,
					undefined,
					{},
				),
