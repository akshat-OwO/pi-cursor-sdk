...
describe("streamCursor trace and streaming", () => {
	...
	it("does not emit pi tool call events for cursor tool deltas", async () => {
		const mockSend = vi
			...
			.mockImplementation(async (_msg: unknown, opts: { onDelta: CursorDeltaHandler }) => {
				opts.onDelta({
					update: { type: "tool-call-started", toolCall: { name: "read_file" }, callId: "c1" },
				});
				opts.onDelta({
					update: { type: "tool-call-completed", toolCall: { name: "read_file" }, callId: "c1" },
				});
	...
	it("surfaces cursor tool results as pi-like trace transcript without polluting final text", async () => {
		...
		expect(done.message.content.map((block) => block.type)).toEqual([
			"thinking",
			"thinking",
			"text",
		]);
	...
	it("uses Cursor onStep tool-call results when delta tool completion is absent", async () => {
		const mockSend = vi
			.fn()
			.mockImplementation(async (_msg: unknown, opts: { onStep: (a: unknown) => void }) => {
				opts.onStep({
					step: {
						type: "toolCall",
						message: {
		...
		const mockSend = vi
			...
			.mockImplementation(async (_msg: unknown, opts: { onDelta: CursorDeltaHandler }) => {
				opts.onDelta({
					update: {
						type: "tool-call-started",
						toolCall: { name: "shell", args: { command: "cat missing.txt" } },
						callId: "c1",
					},
				});
				opts.onDelta({
			...
			.mockImplementation(
				async (_msg: unknown, opts: { onDelta: CursorDeltaHandler; onStep: CursorStepHandler }) => {
					opts.onDelta({
						update: {
							type: "tool-call-started",
							toolCall: { name: "read", args: { path: "missing.txt" } },
							callId: "c1",
						},
					});
					opts.onStep({
		...
		const mockSend = vi
			...
			.mockImplementation(async (_msg: unknown, opts: { onDelta: CursorDeltaHandler }) => {
				...
				return {
					...
					wait: vi
						.fn()
						.mockResolvedValue({ id: "run-1", status: "finished", result: "Final answer." }),
					cancel: vi.fn(),
					supports: () => true,
					unsupportedReason: () => undefined,
				};
	...
	it("trims same-turn final text when streamed text is only a word prefix", async () => {
			...
			.mockImplementation(async (_msg: unknown, opts: { onDelta: CursorDeltaHandler }) => {
				...
				return {
					...
					wait: vi.fn().mockResolvedValue({
						id: "run-1",
						status: "finished",
						result: "Disconnecting the CDP session...",
					}),
					cancel: vi.fn(),
					supports: () => true,
					unsupportedReason: () => undefined,
		...
		expect(done.message.content).toEqual([
			{ type: "text", text: "Disconnecting the CDP session..." },
		]);
	});
		...
		const mockSend = vi
			...
			.mockImplementation(async (_msg: unknown, opts: { onDelta: CursorDeltaHandler }) => {
				opts.onDelta({
					update: {
						type: "tool-call-started",
						toolCall: { name: "read", args: { path: "secrets.txt" } },
						callId: "c1",
					},
				});
				opts.onDelta({
					update: {
						...
						toolCall: {
							...
							result: {
								status: "success",
								value: {
									content: "token=super-secret-key-12345\nAuthorization: Bearer bearer-token-value",
								},
							},
						},
						callId: "c1",
