			...
			.mockImplementation(async (_msg: unknown, opts: { onDelta: CursorDeltaHandler }) => {
				opts.onDelta({
					update: {
						...
						toolCall: { toolName: "run_terminal_cmd", args: { command } },
						callId: "shell-1",
					},
				});
				opts.onDelta({
					update: {
						...
						event: { case: "stdout", value: { data: "background job done\n" } },
					},
				});
				opts.onDelta({
					update: {
						...
						toolCall: {
							...
							result: {
								status: "success",
								value: { stdout: "", stderr: "", exitCode: 0, executionTime: 2015 },
							},
						},
						callId: "shell-1",
					},
		...
		const mockSend = vi
			...
			.mockImplementation(async (_msg: unknown, opts: { onDelta: CursorDeltaHandler }) => {
				opts.onDelta({
					update: {
						type: "tool-call-started",
						toolCall: { name: "shell", args: { command: "sleep 1" } },
						callId: "shell-1",
					},
				});
				opts.onDelta({
					update: {
						type: "shell-output-delta",
						event: { case: "stdout", value: { data: "partial first output\n" } },
					},
				});
				opts.onDelta({
					update: {
						type: "tool-call-started",
						toolCall: { name: "shell", args: { command: "sleep 2" } },
						callId: "shell-2",
					},
				});
				opts.onDelta({
					update: {
						type: "shell-output-delta",
						event: { case: "stdout", value: { data: "ambiguous output\n" } },
					},
				});
					...
					opts.onDelta({
						update: {
							...
							toolCall: {
								...
								result: {
									status: "success",
									value: { stdout: "", stderr: "", exitCode: 0, executionTime: 1 },
								},
							},
							callId,
						},
		...
		const mockSend = vi
			...
			.mockImplementation(async (_msg: unknown, opts: { onDelta: CursorDeltaHandler }) => {
				opts.onDelta({
					update: {
						type: "tool-call-started",
						toolCall: { name: "shell", args: { command: "printf done" } },
						callId: "shell-1",
					},
				});
				opts.onDelta({
					update: {
						type: "shell-output-delta",
						event: { case: "stdout", value: { data: "delta output\n" } },
					},
				});
				opts.onDelta({
					update: {
